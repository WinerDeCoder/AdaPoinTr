...
class PoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.trans_dim = config.trans_dim  # hidden layer của transformer
        self.knn_layer = config.knn_layer  # knn_layer
        self.num_pred = config.num_pred  # số point cloud cần complete
        self.num_query = config.num_query  # M coordinate cua thằng query, treated by as local center của missing point

        self.fold_step = int(pow(self.num_pred//self.num_query, 0.5) + 0.5)  # xác định số foldstep
        ...
        # bỏ point proxy vào transformer, trả về q: query và coarse_point_cloud: point proxy
        self.foldingnet = Fold(self.trans_dim, step = self.fold_step, hidden_dim = 256)  # rebuild a cluster point

        self.increase_dim = nn.Sequential(  # một cái sequential để updim q lên 1024
            nn.Conv1d(self.trans_dim, 1024, 1),
            nn.BatchNorm1d(1024),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Conv1d(1024, 1024, 1)
        )
        self.reduce_map = nn.Linear(self.trans_dim + 1027, self.trans_dim) # reduce dim của 1 thằng trans + 1027 ->trans_dim
