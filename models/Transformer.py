...
def get_graph_feature(x, knn_index, x_q=None):
        # knn_index chắc đã trả về cái idx = idx + idx_base
...
class PCTransformer(nn.Module):
    ...
    def forward(self, inpc):
        ...
        knn_index = get_knn_index(coor)  # for each bs* N . We have 8 points knn --> B, 8, N
        # NOTE: try to use a sin wave  coor B 3 N, change the pos_embed input dim
        # pos = self.pos_encoding_sin_wave(coor).transpose(1,2)
        pos =  self.pos_embed(coor).transpose(1,2)  # B, N, 768
        x = self.input_proj(f).transpose(1,2) # B, N, 768
        # cls_pos = self.cls_pos.expand(bs, -1, -1)
        # cls_token = self.cls_pos.expand(bs, -1, -1)
        # x = torch.cat([cls_token, x], dim=1)
        # pos = torch.cat([cls_pos, pos], dim=1)
        # encoder
        for i, blk in enumerate(self.encoder):    # đoạn này nó đã ra cái output của encoder r
            if i < self.knn_layer:
        ...
        # This is already go to the query generator phase
        global_feature = self.increase_dim(x.transpose(1,2)) # B 1024 N 
        global_feature = torch.max(global_feature, dim=-1)[0] # B 1024

        # Đây là cái đoạn dùng Linear(v) --> Max --> Projection layer --> M x 3 dims
        ...
        # global features of encoder + coarse_point_cloud
        query_feature = torch.cat([
            global_feature.unsqueeze(1).expand(-1, self.num_query, -1), 
            coarse_point_cloud], dim=-1) # B M C+3 
        
        # Xong cho qua thêm 1 lớp MLP to have query
        q = self.mlp_query(query_feature.transpose(1,2)).transpose(1,2) # B M C 
        
        # decoder
        
        # Nó y chang cái encoder, có điều chỗ knn index nó dùng cross thêm từ cái encoder
        ...
        # Point proxy ouput from transformer decoder , M from query
